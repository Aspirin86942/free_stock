...
class OrderExecutionStateSnapshot:
    """单个标的的执行态快照。"""
...
class OrderExecutionStateStore:
    ...
    def __init__(self, logger: Logger | None) -> None:
        self._logger = logger
        self._cache: dict[str, OrderExecutionStateSnapshot] = {}

    def get_state(self, symbol: str) -> OrderExecutionStateSnapshot:
        """读取标的状态；未出现过的标的默认返回 idle。"""
        if symbol not in self._cache:
            return OrderExecutionStateSnapshot(
                symbol=symbol,
                state=OrderExecutionState.idle,
            )
        return self._cache[symbol]

    def active_states(self) -> tuple[OrderExecutionStateSnapshot, ...]:
        """返回当前缓存里的所有执行态快照。"""
