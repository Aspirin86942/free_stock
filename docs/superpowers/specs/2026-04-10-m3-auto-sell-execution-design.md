...
4. 对候选标的计算目标卖量
5. 对目标卖量做市场规则归整
6. 对归整结果做 `available_volume` 校验和重复卖单校验
7. 校验失败则输出阻断事实，不发单
8. 校验通过则进入原子提交流程
9. 同步提交受理成功后，立即进入查询驱动收口链路
10. 后续轮次对在途订单只做跟踪，不重复提交
...
3. 校验归整后数量是否仍为合法申报数量
4. 校验 `normalized_target_volume <= available_volume`
...
- `sell_quantity_exceeds_available` 表示合法申报数量已算出，但超过当前 `available_volume`
