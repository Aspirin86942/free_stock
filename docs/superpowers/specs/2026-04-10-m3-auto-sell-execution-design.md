...
- 若按比例计算后留下的可用仓位残量低于最低申报单位，则优先尝试一次性整可用仓位卖出
...
6. 若整仓卖出不可行，但按比例计算后留下的可用仓位残量低于最低申报单位，则优先尝试整可用仓位卖出
7. 对目标卖量做市场规则归整
8. 对最终目标量做 `available_volume` 校验和重复卖单校验
9. 校验失败则输出阻断事实，不发单
10. 校验通过则进入原子提交流程
11. 同步提交受理成功后，立即进入查询驱动收口链路
12. 后续轮次对在途订单只做跟踪，不重复提交
...
5. 计算按该目标量卖出后的可用仓位残量  
   `available_remaining_after_sell = available_volume - normalized_target_volume`
6. 若 `0 < available_remaining_after_sell < minimum_lot_for_symbol`，则优先把目标量提升为整可用仓位卖出  
   `promoted_target_volume = available_volume`
7. 校验最终目标量是否仍为合法申报数量
8. 校验最终目标量 `<= available_volume`
...
- 若整仓卖出不可行，但原始或归整后的卖量会留下低于最低申报单位的可用仓位残量，则优先尝试整可用仓位卖出
- 若校验失败则直接打回，不自动缩量，不自动补最小单位

这里的“整仓优先 / 整可用仓位优先”是正式规则，不是测试特例。  
例如：

- 总持仓 `250`、原始目标卖量 `201`、总仓剩余 `49`，若 `available_volume >= 250`，则目标量直接提升为 `250`
- 总持仓 `250`、当日可卖 `201`、原始目标卖量 `200`，由于卖出后可用仓位只剩 `1`，则目标量提升为 `201`
...
- 如果整仓不可行，但按当前目标卖出后会留下不足 `100` 股的可用仓位残量，则优先整可用仓位卖出
...
- 如果整仓不可行，但按当前目标卖出后会留下不足 `200` 股的可用仓位残量，则优先整可用仓位卖出
...
- 若整仓提升后的目标量超过 `available_volume`，不自动缩到更小数量
- 但若当前目标卖量会留下低于最低申报单位的可用仓位残量，则允许提升为整可用仓位卖出
...
- 非科创板可用仓位残量低于 `100` 股时提升为整可用仓位卖出
- 科创板可用仓位残量低于 `200` 股时提升为整可用仓位卖出
- 数量归整后为 `0` 的阻断
- 数量超过 `available_volume` 的阻断
- 整仓提升后超过 `available_volume` 时，不回退到任意更小数量
- 整仓提升不可行但可用仓位残量不足最低手数时，会提升为整可用仓位卖出
- 已有未完成卖单时的重复提交拦截
- 同步提交失败时进入 `failed`
- 查询结果驱动的状态迁移
...
- 若当前目标卖量会留下低于最低申报单位的可用仓位残量，M3 会优先整可用仓位卖出，实际卖出比例也可能高于配置比例
