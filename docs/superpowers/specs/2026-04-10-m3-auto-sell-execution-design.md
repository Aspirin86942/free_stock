...
- 若按比例计算后留下的残仓低于最低申报单位，则优先尝试一次性全卖
...
4. 对候选标的计算目标卖量
5. 若按比例计算后留下的残仓低于最低申报单位，则优先把目标量提升为整仓卖出
6. 对目标卖量做市场规则归整
...
2. 计算原始残仓  
   `raw_remaining_volume = position.volume - raw_target_volume`
3. 若 `0 < raw_remaining_volume < minimum_lot_for_symbol`，则优先把目标量提升为整仓卖出  
   `promoted_target_volume = position.volume`
4. 若不满足整仓提升条件，则按标的所属市场规则向下归整  
...
- 若原始卖量会留下低于最低申报单位的残仓，则优先尝试整仓卖出
- 若校验失败则直接打回，不自动缩量，不自动补最小单位

这里的“整仓优先”是正式规则，不是测试特例。  
例如总持仓 `250`、原始目标卖量 `201`、剩余 `49`，由于残仓低于最低申报单位，则目标量直接提升为 `250`。
...
- 也可以把最终剩余零股一次性一起卖掉
- 但不能把零股拆碎
- 如果原始比例卖量会留下不足 `100` 股的残仓，则优先整仓卖出
...
- 如果原始比例卖量会留下不足 `200` 股的残仓，则优先整仓卖出
...
用户已明确确认：即使“整仓优先”后的目标量超过 `available_volume`，也不回退到更小数量，而是直接阻断。
...
- 非科创板数量归整
- 科创板数量归整
- 非科创板残仓低于 `100` 股时提升为整仓卖出
- 科创板残仓低于 `200` 股时提升为整仓卖出
- 数量归整后为 `0` 的阻断
- 数量超过 `available_volume` 的阻断
- 整仓提升后超过 `available_volume` 时直接阻断，不回退到更小数量
- 已有未完成卖单时的重复提交拦截
- 同步提交失败时进入 `failed`
- 查询结果驱动的状态迁移
...
- 若原始比例卖量会留下低于最低申报单位的残仓，M3 会优先整仓卖出，实际卖出比例可能高于配置比例
